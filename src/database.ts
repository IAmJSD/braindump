...
import { braindumpDataDir, braindumpDbDir, pendingImportTarPath } from "./dataTransfer";
...
declare global {
    ...
    var __APPLIED_PENDING_IMPORT__: boolean | undefined;
}

function getPendingImportBlob(): Blob | undefined {
    if (!existsSync(pendingImportTarPath)) return undefined;
    globalThis.__APPLIED_PENDING_IMPORT__ = true;
    return new Blob([readFileSync(pendingImportTarPath)]);
}

export function clearPendingImportBackup() {
    if (!existsSync(pendingImportTarPath)) return;
    rmSync(pendingImportTarPath);
}

mkdirSync(braindumpDataDir, { recursive: true });
const pendingImportBlob = getPendingImportBlob();
if (pendingImportBlob && existsSync(braindumpDbDir)) {
    // PGlite cannot load a tar backup into an existing data directory.
    rmSync(braindumpDbDir, { recursive: true, force: true });
}
export const pgLite = new PGlite(braindumpDbDir, {
    extensions: { vector },
    ...(pendingImportBlob ? { loadDataDir: pendingImportBlob } : {}),
});
