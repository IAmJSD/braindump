...
function SettingsContent({ onClose }: { onClose: () => void }) {
    ...
    const handleExportData = async () => {
        ...
        try {
            const result = await rpcMethods.exportData(undefined);
            if (result.cancelled) return;
            if (!result.success) {
                setTransferError("Export failed. Please try again.");
                return;
            }
            setTransferMessage(`Backup exported to ${result.path}.`);
        } catch {
            setTransferError("Export failed. Please try again.");
        } finally {
    ...
    const handleImportData = async () => {
        ...
        setTransferMessage(null);
        try {
            const result = await rpcMethods.importData(undefined);
            if (result.cancelled) return;
            if (!result.success) {
                setTransferError("Import failed. Please try again.");
                return;
            }
            setTransferMessage("Import queued successfully. Restart Braindump to apply it.");
        } catch {
            setTransferError("Import failed. Please try again.");
        } finally {
            setTransferBusy(null);
    ...
    return (
        <div className="p-6">
            ...
            {/* Data transfer */}
            <div>
                <span className="text-xs text-gray-500 uppercase tracking-wider font-medium block mb-2">Data</span>
                <div className="grid grid-cols-2 gap-2">
                    <button
                        onClick={handleExportData}
                        disabled={transferBusy !== null}
                        className="px-3 py-2.5 rounded-xl bg-gray-50 dark:bg-gray-800 hover:bg-gray-100 dark:hover:bg-gray-700 disabled:opacity-40 disabled:cursor-not-allowed text-sm text-gray-800 dark:text-gray-200 transition-colors"
                    >
                        {transferBusy === "export" ? "Exporting..." : "Export data"}
                    </button>
                    <button
                        ...
                        className="px-3 py-2.5 rounded-xl bg-gray-50 dark:bg-gray-800 hover:bg-gray-100 dark:hover:bg-gray-700 disabled:opacity-40 disabled:cursor-not-allowed text-sm text-gray-800 dark:text-gray-200 transition-colors"
                    >
                        {transferBusy === "import" ? "Importing..." : "Import data"}
                    </button>
                </div>
                {transferMessage && <p className="text-xs text-emerald-500 mt-2">{transferMessage}</p>}
                {transferError && <p className="text-xs text-red-400 mt-2">{transferError}</p>}
            </div>
        </div>
    );
}
